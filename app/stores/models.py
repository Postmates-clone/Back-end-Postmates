...
class Store(models.Model):
    ...
    delivery_message = models.TextField(
        blank=True, null=True, help_text='주문 가능 메세지')
    last_order_time = models.FloatField(
        blank=True, null=True, help_text='라스트 오더 시간')
    is_delivery = models.BooleanField(
        help_text='배달 여부')
    is_pickup = models.BooleanField(
        blank=True, null=True, help_text='테이크아웃 여부')
    delivery_fee = models.FloatField(
        default=0, help_text='배달 비')
    estimated_prep_time = models.IntegerField(
        default=0, help_text='예상 준비 시간')
    is_partner = models.BooleanField(
        blank=True, null=True, help_text='파트너인지 여부')
...
class Menu(models.Model):
    name = models.TextField(
        help_text='음식 이름')
    description = models.TextField(
        blank=True, null=True, help_text='음식 설명')
    price = models.FloatField(
        blank=True, null=True, help_text='음식 가격')
    image_url = models.TextField(
        blank=True, null=True, help_text='음식 대표 이미지')
...
class OptionCategory(models.Model):
    ...
    name = models.TextField(
        unique=True, help_text='옵션 카테고리 이름')
